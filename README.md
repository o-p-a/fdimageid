...
* `--convert-d88`  
  D88形式への変換を行います。トラックエントリの数は164となります。1.44MBフォーマットのデータの場合、ディスクタイプは21hを設定します。また、各セクタのNP2kai RPMフラグに1を設定します。出力ファイルのファイル名は、入力となったファイル名に`.d88`を付加したものになります。(つまり、`….xdf.d88`のようになります)
